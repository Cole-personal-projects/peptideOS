import type { ReferenceRegistrySeed } from './reference-registry-seed';

export function buildReferenceRegistryImportSql(seed: ReferenceRegistrySeed): string {
...
function insertCitations(rows: ReferenceRegistrySeed['citations']): string {
...
function insertCompounds(rows: ReferenceRegistrySeed['compounds']): string {
  return [
...
function insertAliases(rows: ReferenceRegistrySeed['aliases']): string {
  if (rows.length === 0) return '';
  return [
...
function insertCategories(rows: ReferenceRegistrySeed['categories']): string {
  return [
...
function insertForms(rows: ReferenceRegistrySeed['forms']): string {
  return [
...
function insertCompoundCitations(rows: ReferenceRegistrySeed['compoundCitations']): string {
  if (rows.length === 0) return '';
  return [
...
function insertDosePresets(rows: ReferenceRegistrySeed['dosePresets']): string {
  if (rows.length === 0) return '';
  return [
...
function insertVialPresets(rows: ReferenceRegistrySeed['vialPresets']): string {
  if (rows.length === 0) return '';
  return [
...
function insertWorkflowMetadata(rows: ReferenceRegistrySeed['workflowMetadata']): string {
  return [
...
function insertContentBlocks(rows: ReferenceRegistrySeed['contentBlocks']): string {
  if (rows.length === 0) return '';
  return [
...
function insertLibraryRelease(row: ReferenceRegistrySeed['libraryRelease']): string {
  return [
...
function insertReleaseItems(rows: ReferenceRegistrySeed['releaseItems']): string {
  if (rows.length === 0) return '';
  return [
...
function compoundId(slug: string): string {
  return `(select id from public.reference_compounds where slug = ${sqlString(slug)})`;
}

function sqlString(value: string | number | boolean): string {
...
function sqlNumericArray(values: number[]): string {
