...
type RecordEvidenceTier = 'identity_only' | 'preclinical' | 'human_limited' | 'human_strong' | 'approved_label';
...
export function referenceLibraryRecordToCompound(record: ReferenceLibraryRecord): ReferenceCompound {
  ...
  const clinicalEvidence = record.claims.map(claimToClinicalEvidence);
  ...
  const compound: ReferenceCompound = {
    ...
    category: record.identity.categories[0] ?? 'custom',
    ...
    referenceProfile: {
      ...
      regulatoryStatus: {
        status: record.evidence.tier === 'approved_label' ? 'approved' : 'unknown',
        region: 'US',
        summary: record.evidence.tier === 'approved_label'
        ...
        limitations: 'Regulatory status must be verified from source-backed records before display as approved.',
      },
